(function(){
  ...
  function panel(r){
    ...
    const fields=(r.localCards||r.fields||[]).map(f=>{const label=String(f[0]||'');const value=String(f[1]??'');const descriptive=value.length>28||/boundary|scope|note|description|approval|status/i.test(label);return '<div class="'+(descriptive?'br-card-text':'')+'"><span>'+escape(label)+'</span><strong>'+escape(value)+'</strong></div>';}).join('');
