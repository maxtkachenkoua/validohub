(function(){
  ...
  function onMount(w){injectStyles(); const c=cfg(); w.form.classList.add('brazil-suite-workbench'); const input=w.primaryInput(); if(input){input.placeholder='Paste '+c.title+' input, batch rows, or a Brazilian payload...';} const row=document.createElement('div'); row.className='br-tool-head'; row.innerHTML='<div><span>'+escape(c.code)+'</span><strong>'+escape(c.title)+'</strong><small>'+escape(c.summary)+'</small></div><select class="br-preset"><option value="">Samples and fixtures</option><option value="sample">Representative sample</option><option value="cpf">CPF</option><option value="cnpj">CNPJ</option><option value="cep">CEP</option><option value="boleto">Boleto</option><option value="nfe">NF-e key</option><option value="money">BRL amount</option></select>'; w.form.insertBefore(row,w.form.firstChild); row.querySelector('select').addEventListener('change',e=>{if(e.target.value)applySample(w,e.target.value);}); }
