(function () {
  ...
  const Plugin = (function (framework) {
    ...
    TOOLS = {
      'poland-nip-validator': { title: 'NIP Validator & Explainer', label: 'NIP', samples: ['1234563218', '123-456-32-18', '1234563210'], generator: generateNip, analyze: analyzeNip, family: 'Tax identifier', mask: maskPolishNumber, fieldLabel: 'NIP digits' },
      'poland-regon-validator': { title: 'REGON Validator & Explainer', label: 'REGON', samples: ['123456785', '12345678512347', '123456789'], generator: generateRegon9, analyze: analyzeRegon, family: 'Business register', mask: maskPolishNumber, fieldLabel: 'REGON digits' },
      'poland-iban-nrb-validator': { title: 'Polish IBAN / NRB Workbench', label: 'IBAN / NRB', samples: ['PL61109010140000071219812874', '61109010140000071219812874', 'PL00109010140000071219812874'], generator: generateNrb, analyze: analyzeIbanNrb, family: 'Banking', mask: maskIban, fieldLabel: 'Account number' },
      'poland-tax-microaccount-calculator': { title: 'Polish Tax Microaccount Calculator', label: 'PESEL or NIP', samples: ['44051401458', '1234563218', '1234563210'], generator: generateNip, analyze: analyzeTaxMicro, family: 'Tax payment', mask: maskPolishNumber, fieldLabel: 'Source identifier' },
      'poland-postal-code-validator': { title: 'Polish Postal Code Validator', label: 'Postal code', samples: ['00-001', '00001', '0A-001'], generator: generatePostal, analyze: analyzePostal, family: 'Address', mask: value => value, fieldLabel: 'Postal code' },
      'poland-phone-number-validator': { title: 'Polish Phone Number Workbench', label: 'Phone number', samples: ['+48 501 234 567', '22 123 45 67', '801 234 567', '12345'], generator: generatePhone, analyze: analyzePhone, family: 'Telecom', mask: maskPhone, fieldLabel: 'Phone number' },
      'poland-license-plate-inspector': { title: 'Polish License Plate Inspector', label: 'License plate', samples: ['WA12345', 'KR 1A234', 'PO1234A', 'ZZ99999'], generator: generatePlate, analyze: analyzePlate, family: 'Vehicle', mask: maskPlate, fieldLabel: 'License plate' },
      'poland-krs-inspector': { title: 'KRS Number Inspector', label: 'KRS number', samples: ['0000123456', '1234567890', '12345'], generator: generateKrs, analyze: analyzeKrs, family: 'Company register', mask: maskPolishNumber, fieldLabel: 'KRS digits' },
      'poland-vat-validator': { title: 'Polish VAT / EU VAT Syntax Workbench', label: 'VAT number', samples: ['PL1234563218', '1234563218', 'PL1234563210'], generator: () => 'PL' + generateNip(), analyze: analyzeVat, family: 'EU VAT syntax', mask: maskVat, fieldLabel: 'VAT syntax' },
      'poland-bank-code-inspector': { title: 'Polish Bank Code / NRB Inspector', label: 'IBAN / NRB', samples: ['PL61109010140000071219812874', '61109010140000071219812874', 'PL00109010140000071219812874'], generator: generateNrb, analyze: analyzeBankCode, family: 'Banking', mask: maskIban, fieldLabel: 'Bank account' }
    };
    ...
    function enhanceIntro(config) {
      ...
      if (summary) summary.textContent = 'Validate, batch-check, mask, generate safe fixtures, and inspect Polish data locally with PESEL-grade diagnostics.';
      if (!intro.querySelector('.poland-badge-row')) {
        intro.insertAdjacentHTML('beforeend', '<div class="poland-badge-row"><span class="poland-pill active">Local Sandbox</span><span class="poland-pill">Batch Mode</span><span class="poland-pill">Masking</span><span class="poland-pill">Audit JSON</span><span class="poland-pill">No Lookup</span></div>');
      }
    }
