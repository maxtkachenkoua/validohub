(function () {
  ...
  const Plugin = (function (framework) {
    ...
    function addToolHeader(workbench, config) {
      if (workbench.form.querySelector('[data-poland-tool-head]')) return;
      const sampleChips = (config.samples || []).slice(0, 3).map((sample, index) => '<button type="button" data-poland-header-sample="' + index + '">' + util.escapeHtml(sample) + '</button>').join('');
      const options = (config.samples || []).map((sample, index) => '<option value="' + index + '">' + escapeAttr(sample) + '</option>').join('');
      const head = document.createElement('section');
      head.className = 'poland-tool-head';
      head.setAttribute('data-poland-tool-head', '');
      head.innerHTML = '<div class="poland-tool-identity"><span class="poland-tool-mark">' + util.escapeHtml(config.label || 'PL') + '</span><div><strong>' + util.escapeHtml(config.title) + '</strong><small>' + util.escapeHtml(config.summary) + '</small></div></div><div class="poland-tool-presets"><label><span>Smart presets</span><select data-poland-preset><option value="">Choose a ' + escapeAttr(config.label) + ' sample</option>' + options + '</select></label><label><span>Recent local inputs</span><select data-poland-history><option value="">No history yet</option></select></label><div class="poland-tool-samples">' + sampleChips + '</div></div>';
    ...
    function addControls(workbench, config) {
      ...
      if (!grid) return;
      if (!workbench.form.querySelector('[data-poland-preset]')) {
    ...
    function bindEvents(workbench, config) {
      ...
      workbench.form.addEventListener('click', event => {
        const sample = event.target.closest('[data-poland-header-sample]');
        if (sample) applySample(workbench, sample.dataset.polandHeaderSample);
    ...
    function renderResult(workbench, config, result) {
      ...
      renderSummary(workbench, [card('Type', result.type, result.valid ? 'success':'error'), card('Normalized', result.normalized || 'n/a', 'mono'), card('Masked', result.masked || 'n/a', 'mono'), card('Context', result.context || config.family, '')], result);
    ...
    function renderTimeline(workbench, stages) {
      ...
      target.innerHTML = '<section class="poland-pipeline"><div class="poland-section-title"><span>◎</span> Validation Pipeline</div><div class="poland-timeline-track"><span style="width:' + width + '%"></span></div><div class="poland-pipeline-list">' + stages.map(s => '<article class="poland-pipeline-step ' + s.state + '"><div class="poland-step-header"><strong>' + util.escapeHtml(s.label) + '</strong><span>' + (s.state === 'pass' ? 'PASS' : s.state === 'fail' ? 'REVIEW' : 'READY') + '</span></div><p>' + util.escapeHtml(s.note) + '</p></article>').join('') + '</div></section>';
    }
    function renderSummary(workbench, cards, result) { const target = workbench.form.querySelector('[data-poland-summary]'); if (!target) return; const checks = result ? [result.valid ? 'Offline structural checks passed.' : 'Review the highlighted diagnostics.', result.proves || 'Syntax and checksum analysis complete.'] : []; target.innerHTML = '<section class="poland-results-container ' + (result && result.valid ? 'success' : 'error') + '"><div class="poland-results-header"><span>' + (result && result.valid ? '✓' : '!') + '</span><strong>' + (result && result.valid ? 'Validated locally' : 'Needs review') + '</strong><em>' + util.escapeHtml(result ? (result.proves || 'Offline analysis complete') : 'Ready') + '</em></div><div class="poland-results-grid">' + cards.map(c => { const value = String(c.value || ''); const long = value.length > 18 ? ' is-long' : ''; return '<article class="poland-summary-card ' + (c.state || '') + long + '"><span>' + util.escapeHtml(c.label) + '</span><strong>' + util.escapeHtml(value) + '</strong></article>'; }).join('') + '</div><div class="poland-result-checks">' + checks.map(item => '<p><span>✓</span>' + util.escapeHtml(item) + '</p>').join('') + '</div></section>'; }
    ...
    function breakdownHtml(r){ const bad=new Set(r.problemIndexes||[]); const chars=Array.from(r.normalized||'').map((c,i)=>'<span class="tone-'+(i%6)+' '+(bad.has(i)?'is-problem':'')+'" title="Position '+(i+1)+'"><b>'+util.escapeHtml(c)+'</b><em>'+String(i+1)+'</em></span>').join(''); return '<section class="poland-section-card poland-breakdown-card"><div class="poland-section-title"><span>▥</span> Identifier Breakdown</div><div class="poland-token-row">'+(chars || '<p class="poland-muted">No normalized value yet.</p>')+'</div><div class="poland-token-help">Position-by-position normalized value, with problem characters highlighted when a check fails.</div><div class="poland-detail-grid"><div><span>Input</span><strong>'+util.escapeHtml(r.input)+'</strong></div><div><span>Normalized</span><strong>'+util.escapeHtml(r.normalized||'n/a')+'</strong></div><div><span>Masked</span><strong>'+util.escapeHtml(r.masked||'n/a')+'</strong></div><div><span>Boundary</span><strong>'+util.escapeHtml(r.boundary||'Offline only')+'</strong></div></div>'+(r.fields&&r.fields.length?'<div class="poland-field-table">'+r.fields.map(f=>'<div><span>'+util.escapeHtml(f[0])+'</span><strong>'+util.escapeHtml(f[1])+'</strong></div>').join('')+'</div>':'')+'</section>';}
    ...
    function debuggerHtml(r){ const parsed = parseWeightedSteps(r.steps || []); const table = parsed.length ? '<div class="poland-debugger-table-wrap"><table class="poland-debugger-table"><thead><tr><th>Position</th><th>Digit</th><th>Weight</th><th>Product</th><th>Running sum</th></tr></thead><tbody>'+parsed.map(row=>'<tr><td>d'+row.position+'</td><td>'+row.digit+'</td><td>'+row.weight+'</td><td>'+row.product+'</td><td>'+row.running+'</td></tr>').join('')+'</tbody></table></div>' : ''; const steps = (!parsed.length && r.steps.length) ? '<div class="poland-step-cards">' + r.steps.map((step,index)=>'<article><span>'+(index+1)+'</span><p>'+util.escapeHtml(step)+'</p></article>').join('') + '</div>' : ''; const diagnostics = r.diagnostics.length ? '<div class="poland-diagnostic-cards">' + r.diagnostics.map(s=>'<article>'+util.escapeHtml(s)+'</article>').join('') + '</div>' : '<p class="poland-muted">No offline structural issues found.</p>'; const formula = r.steps.find(step => /Weighted sum|MOD-97|mod/i.test(step)); return '<section class="poland-section-card poland-debugger-card"><div class="poland-section-title"><span>▦</span> Checksum Debugger</div><div class="poland-formula-line">'+util.escapeHtml(formula || 'Step-by-step offline calculation trace.')+'</div><div class="poland-detail-grid"><div><span>Expected</span><strong>'+util.escapeHtml(r.expected||'n/a')+'</strong></div><div><span>Provided</span><strong>'+util.escapeHtml(r.provided||'n/a')+'</strong></div><div><span>Status</span><strong class="'+(r.valid?'poland-ok':'poland-bad')+'">'+(r.valid?'Pass':'Review')+'</strong></div><div><span>Scope</span><strong>Offline structural check</strong></div></div>'+table+steps+diagnostics+'</section>';}
    function parseWeightedSteps(steps){ let running=0; return steps.map(step=>{ const m=String(step).match(/Position (\d+): (\d+) x weight (\d+) = (\d+)/); if(!m)return null; running+=Number(m[4]); return { position:m[1], digit:m[2], weight:m[3], product:m[4], running:String(running) }; }).filter(Boolean); }
    function devHtml(config,r){ return '<section class="poland-section-card poland-advanced-card"><div class="poland-section-title"><span>▸</span> Advanced Analysis</div><details class="poland-dev-accordion" open><summary>Developer snapshot JSON</summary><div class="poland-dev-accordion-content"><button type="button" class="poland-dev-copy" data-poland-copy="json">Copy</button><pre class="poland-code"><code>'+syntaxHighlightJson(publicJson(config,r))+'</code></pre></div></details><details class="poland-dev-accordion" open><summary>Validation boundary</summary><div class="poland-dev-accordion-content"><p>'+util.escapeHtml(r.boundary || 'Official live status is not checked.')+'</p></div></details></section>';}
    ...
    function injectStyles(){ if(document.getElementById('poland-suite-styles'))return; const style=document.createElement('style'); style.id='poland-suite-styles'; style.textContent=".poland-suite-workbench{--pl-accent:#9f1239;--pl-soft:#fff1f2;--pl-accent-strong:#be123c;--pl-green:#16a34a;--pl-bad:#dc2626;--pl-amber:#d97706}.poland-tool-head{display:grid;grid-template-columns:minmax(0,1fr) minmax(300px,420px);gap:18px;align-items:stretch;border:1px solid color-mix(in srgb,var(--pl-accent) 20%,var(--line));border-radius:16px;padding:18px;margin-bottom:20px;background:linear-gradient(110deg,#fff 0%,var(--pl-soft) 52%,rgba(255,255,255,.92) 100%);box-shadow:0 18px 42px rgba(15,23,42,.06);position:relative;overflow:hidden}.poland-tool-head:before{content:'';position:absolute;inset:0;background:linear-gradient(90deg,rgba(220,38,38,.12),rgba(255,255,255,.5) 45%,rgba(15,23,42,.05));pointer-events:none}.poland-tool-identity,.poland-tool-presets{position:relative;z-index:1}.poland-tool-identity{display:grid;grid-template-columns:auto minmax(0,1fr);gap:14px;align-items:start}.poland-tool-mark{display:inline-grid;place-items:center;min-width:58px;height:48px;border-radius:12px;border:1px solid color-mix(in srgb,var(--pl-accent) 24%,var(--line));background:linear-gradient(135deg,#fff,var(--pl-soft));color:var(--pl-accent-strong);font-weight:950;letter-spacing:.08em;font-size:.72rem;text-transform:uppercase;box-shadow:0 8px 20px rgba(15,23,42,.05);padding:0 10px}.poland-tool-identity strong{display:block;font-size:1.05rem;line-height:1.2;color:var(--text)}.poland-tool-identity small{display:block;margin-top:6px;max-width:780px;color:var(--muted);line-height:1.45}.poland-tool-presets{display:grid;gap:10px;align-content:start}.poland-tool-presets label span{display:block;font-size:.66rem;font-weight:950;text-transform:uppercase;letter-spacing:.09em;color:var(--muted);margin-bottom:5px}.poland-tool-presets select{width:100%;border:1px solid var(--line);border-radius:10px;background:#fff;color:var(--text);font-weight:850;padding:10px 12px}.poland-tool-samples{display:flex;flex-wrap:wrap;gap:8px}.poland-tool-samples button{border:1px solid color-mix(in srgb,var(--pl-accent) 24%,var(--line));border-radius:999px;background:#fff;color:var(--pl-accent-strong);font-weight:850;font-size:.74rem;padding:7px 10px;max-width:100%;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;cursor:pointer}.poland-tool-samples button:hover{background:var(--pl-soft);transform:translateY(-1px)}.poland-badge-row,.poland-detail-grid,.poland-summary-grid{display:flex;flex-wrap:wrap;gap:8px}.poland-badge-row{margin-top:16px}.poland-pill{font-size:.72rem;font-weight:800;text-transform:uppercase;letter-spacing:.055em;padding:6px 11px;border-radius:999px;border:1px solid color-mix(in srgb,var(--pl-accent) 22%,var(--line));background:var(--surface-soft);color:var(--muted)}.poland-pill.active{color:var(--pl-accent-strong);border-color:color-mix(in srgb,var(--pl-accent) 36%,var(--line));background:var(--pl-soft)}.poland-select-field{min-width:220px}.poland-panel{margin-top:24px;display:flex;flex-direction:column;gap:24px}.poland-empty-state{border:1px dashed color-mix(in srgb,var(--pl-accent) 28%,var(--line));border-radius:12px;padding:30px;background:linear-gradient(135deg,var(--pl-soft),#fff);display:flex;align-items:center;gap:14px;color:var(--muted)}.poland-empty-icon{width:54px;height:54px;border-radius:16px;display:grid;place-items:center;color:#fff;background:linear-gradient(135deg,var(--pl-accent),var(--pl-accent-strong));font-weight:900}.poland-empty-state strong{color:var(--text);display:block;margin-bottom:4px}.poland-batch-card{border:1px solid var(--line);border-radius:10px;background:#fff;margin:16px 0;padding:0;overflow:hidden}.poland-batch-card summary{cursor:pointer;padding:14px 16px;display:flex;align-items:center;justify-content:space-between;gap:12px;font-weight:900}.poland-batch-card summary em{font-style:normal;color:var(--muted);font-weight:700;font-size:.78rem}.poland-batch-card .field,.poland-batch-actions,.poland-batch-results{margin:0 16px 16px}.poland-batch-actions{display:flex;flex-wrap:wrap;gap:10px}.poland-batch-summary{display:flex;gap:10px;flex-wrap:wrap;margin-bottom:10px}.poland-batch-summary>*{border:1px solid var(--line);border-radius:999px;padding:6px 10px;background:var(--surface-soft)}.poland-batch-table{display:grid;gap:6px}.poland-batch-table>div{display:grid;grid-template-columns:44px minmax(0,1fr) 80px minmax(0,1fr);gap:8px;align-items:center;border:1px solid var(--line);border-radius:8px;padding:8px}.poland-batch-table code,.poland-batch-table em{overflow:hidden;text-overflow:ellipsis;white-space:nowrap}.poland-batch-table .pass strong{color:var(--pl-green)}.poland-batch-table .fail strong{color:var(--pl-bad)}.poland-pipeline,.poland-results-container,.poland-section-card{border:1px solid var(--line);border-radius:12px;background:#fff;padding:20px;overflow:hidden;box-shadow:0 10px 28px rgba(15,23,42,.035)}.poland-pipeline{background:linear-gradient(180deg,#fff,var(--surface-soft))}.poland-section-title{font-size:.9rem;font-weight:900;letter-spacing:.07em;text-transform:uppercase;border-bottom:1px solid var(--line);padding-bottom:14px;margin:0 0 18px;display:flex;align-items:center;gap:10px;color:var(--text)}.poland-section-title span{color:var(--pl-accent)}.poland-timeline-track{height:3px;background:#e5e7eb;margin:8px 24px 22px;border-radius:999px;overflow:hidden}.poland-timeline-track span{display:block;height:100%;max-width:100%;background:linear-gradient(90deg,var(--pl-accent),var(--pl-green))}.poland-pipeline-list{display:grid;grid-template-columns:repeat(auto-fit,minmax(220px,1fr));gap:12px}.poland-pipeline-step{border:1px solid var(--line);border-radius:10px;padding:14px;background:#fff;min-width:0}.poland-pipeline-step.pass{border-color:rgba(22,163,74,.24);background:linear-gradient(135deg,rgba(22,163,74,.055),#fff)}.poland-pipeline-step.fail{border-color:rgba(220,38,38,.24);background:linear-gradient(135deg,rgba(220,38,38,.055),#fff)}.poland-step-header{display:flex;align-items:flex-start;justify-content:space-between;gap:10px}.poland-step-header strong{font-size:.9rem}.poland-step-header span{font-size:.68rem;font-weight:900;letter-spacing:.08em;border-radius:8px;padding:4px 8px;background:var(--pl-soft);color:var(--pl-accent-strong)}.poland-pipeline-step.pass .poland-step-header span{background:rgba(22,163,74,.1);color:var(--pl-green)}.poland-pipeline-step.fail .poland-step-header span{background:rgba(220,38,38,.1);color:var(--pl-bad)}.poland-pipeline-step p{margin:12px 0 0;color:var(--muted);font-size:.86rem;line-height:1.45;overflow-wrap:anywhere}.poland-results-container{background:linear-gradient(135deg,rgba(22,163,74,.055),#fff);border-color:rgba(22,163,74,.22)}.poland-results-container.error{background:linear-gradient(135deg,rgba(220,38,38,.055),#fff);border-color:rgba(220,38,38,.22)}.poland-results-header{display:flex;align-items:center;gap:12px;margin-bottom:14px;color:var(--text)}.poland-results-header span{width:32px;height:32px;border-radius:999px;display:grid;place-items:center;background:rgba(22,163,74,.12);color:var(--pl-green);font-weight:900}.poland-results-container.error .poland-results-header span{background:rgba(220,38,38,.12);color:var(--pl-bad)}.poland-results-header strong{font-size:1rem}.poland-results-header em{font-style:normal;color:var(--muted);font-size:.82rem}.poland-results-grid{display:grid;grid-template-columns:repeat(4,minmax(0,1fr));gap:12px}.poland-summary-card{border:1px solid var(--line);border-radius:10px;background:#fff;padding:16px;min-width:0;overflow:hidden}.poland-summary-card span,.poland-detail-grid span,.poland-field-table span{display:block;color:var(--muted);font-size:.68rem;font-weight:900;letter-spacing:.07em;text-transform:uppercase;margin-bottom:8px}.poland-summary-card strong,.poland-detail-grid strong,.poland-field-table strong{display:block;max-width:100%;min-width:0;overflow-wrap:anywhere;word-break:break-word;line-height:1.28}.poland-summary-card.is-long strong{font-size:clamp(.72rem,1.3vw,.94rem)}.poland-summary-card.success strong,.poland-ok{color:var(--pl-green)}.poland-summary-card.error strong,.poland-bad{color:var(--pl-bad)}.poland-summary-card.mono strong{font-family:ui-monospace,SFMono-Regular,Menlo,monospace}.poland-result-checks{margin-top:16px;border-top:1px solid var(--line);padding-top:12px}.poland-result-checks p{display:flex;align-items:center;gap:10px;margin:8px 0;color:var(--muted)}.poland-result-checks span{color:var(--pl-green);font-weight:900}.poland-token-row{display:flex;flex-wrap:wrap;justify-content:center;gap:8px;margin:10px 0 18px}.poland-token-row span{border:1px solid var(--line);border-radius:10px;min-width:42px;padding:10px 9px;text-align:center;font-family:ui-monospace,SFMono-Regular,Menlo,monospace;background:#fff;font-weight:900;box-shadow:0 8px 18px rgba(15,23,42,.045)}.poland-token-row b{display:block;font-size:1.18rem}.poland-token-row em{display:block;font-style:normal;font-size:.58rem;color:var(--muted);margin-top:2px}.poland-token-row .tone-0{border-color:rgba(47,128,237,.36);color:#2f80ed}.poland-token-row .tone-1{border-color:rgba(16,185,129,.36);color:#10b981}.poland-token-row .tone-2{border-color:rgba(245,158,11,.4);color:#d97706}.poland-token-row .tone-3{border-color:rgba(139,92,246,.36);color:#7c3aed}.poland-token-row .tone-4{border-color:rgba(236,72,153,.36);color:#db2777}.poland-token-row .tone-5{border-color:rgba(14,165,233,.36);color:#0284c7}.poland-token-row span.is-problem{border-color:rgba(220,38,38,.55);background:rgba(220,38,38,.08);color:var(--pl-bad)}.poland-token-help{border:1px solid var(--line);border-radius:10px;background:var(--surface-soft);padding:13px 16px;color:var(--muted);margin-bottom:16px}.poland-detail-grid,.poland-field-table{display:grid;grid-template-columns:repeat(4,minmax(0,1fr));gap:10px}.poland-field-table{margin-top:10px}.poland-detail-grid>div,.poland-field-table>div{border:1px solid var(--line);border-radius:10px;padding:14px;min-width:0;overflow:hidden;background:#fff}.poland-quality-list{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:10px}.poland-quality-list article{border:1px solid var(--line);border-radius:10px;padding:14px;background:var(--surface-soft)}.poland-quality-list article.ok{border-color:rgba(22,163,74,.25);background:rgba(22,163,74,.06)}.poland-quality-list article.warn{border-color:rgba(217,119,6,.25);background:rgba(217,119,6,.06)}.poland-quality-list article.tip{border-color:rgba(37,99,235,.2);background:rgba(37,99,235,.05)}.poland-quality-list span{font-size:.68rem;font-weight:900;letter-spacing:.07em;text-transform:uppercase;color:var(--muted)}.poland-quality-list p{margin:.35rem 0 0;color:var(--text)}.poland-formula-line{font-family:ui-monospace,SFMono-Regular,Menlo,monospace;color:var(--muted);margin-bottom:16px;overflow-wrap:anywhere}.poland-debugger-table-wrap{overflow:auto;border:1px solid var(--line);border-radius:10px;margin-top:14px}.poland-debugger-table{width:100%;border-collapse:collapse;font-family:ui-monospace,SFMono-Regular,Menlo,monospace;font-size:.82rem}.poland-debugger-table th,.poland-debugger-table td{padding:10px 12px;border-bottom:1px solid var(--line);text-align:left}.poland-debugger-table th{font-size:.68rem;letter-spacing:.07em;text-transform:uppercase;color:var(--muted);background:var(--surface-soft)}.poland-step-cards,.poland-diagnostic-cards{display:grid;grid-template-columns:repeat(auto-fit,minmax(220px,1fr));gap:8px;margin-top:12px}.poland-step-cards article,.poland-diagnostic-cards article{display:grid;grid-template-columns:28px minmax(0,1fr);gap:10px;align-items:start;border:1px solid var(--line);border-radius:10px;background:var(--surface-soft);padding:10px}.poland-step-cards article span{display:grid;place-items:center;width:24px;height:24px;border-radius:999px;background:var(--pl-soft);color:var(--pl-accent-strong);font-size:.72rem;font-weight:900}.poland-step-cards p{margin:0;color:var(--muted);font-size:.8rem;line-height:1.4;overflow-wrap:anywhere}.poland-diagnostic-cards article{grid-template-columns:1fr;border-color:rgba(220,38,38,.25);background:rgba(220,38,38,.05);color:#991b1b;font-size:.84rem}.poland-dev-accordion{border:1px solid var(--line);border-radius:10px;background:#fff;overflow:hidden;margin-top:10px}.poland-dev-accordion summary{cursor:pointer;list-style:none;padding:16px 18px;font-weight:900;text-transform:uppercase;letter-spacing:.06em;display:flex;justify-content:space-between;gap:12px}.poland-dev-accordion summary::-webkit-details-marker{display:none}.poland-dev-accordion summary::after{content:'▾';color:var(--muted)}.poland-dev-accordion[open] summary::after{content:'▴'}.poland-dev-accordion-content{border-top:1px solid var(--line);padding:16px;position:relative;background:var(--surface-soft)}.poland-dev-copy{position:absolute;right:14px;top:14px;border:1px solid var(--line);border-radius:8px;background:#fff;padding:5px 10px;color:var(--muted);font-weight:800}.poland-muted{color:var(--muted)}.poland-code{margin:0;overflow:auto;padding:14px;border-radius:8px;background:#0f172a;color:#dbeafe;font-size:.78rem}.poland-json-key{color:#93c5fd}.poland-json-string{color:#86efac}.poland-json-number{color:#fbbf24}.poland-json-bool{color:#f0abfc}.poland-json-null{color:#cbd5e1}@media(max-width:900px){.poland-tool-head{grid-template-columns:1fr}.poland-tool-identity{grid-template-columns:1fr}.poland-tool-mark{width:max-content}.poland-tool-samples button{white-space:normal;text-align:left}.poland-results-grid,.poland-detail-grid,.poland-field-table,.poland-quality-list{grid-template-columns:1fr}.poland-timeline-track{display:none}.poland-empty-state{align-items:flex-start}.poland-batch-card summary{align-items:flex-start;flex-direction:column}.poland-batch-table>div{grid-template-columns:36px minmax(0,1fr);}.poland-batch-table em{grid-column:2}.poland-step-cards,.poland-diagnostic-cards{grid-template-columns:1fr}.poland-results-header{align-items:flex-start;flex-direction:column}.poland-token-row{justify-content:flex-start}}"; document.head.appendChild(style); }
