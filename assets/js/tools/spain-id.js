(function () {
  ...
  const SpanishIdPlugin = (function (framework) {
    ...
    function onMount(workbench) {
      ...
      const hasQueryValue = hydrateFromQuery(workbench);
      if (!hasQueryValue) {
        setInput(workbench, DEFAULT_SAFE_FIXTURE);
    ...
    function normalizeControls(workbench) {
      ...
      if (buttonRow && !buttonRow.querySelector('[data-spain-action="batch"]')) {
        buttonRow.insertAdjacentHTML('beforeend', '<button type="button" class="button button-secondary" data-spain-action="batch">Batch replay</button>');
      }
    }
    ...
    function hydrateFromQuery(workbench) {
      ...
      if (value) { setInput(workbench, value); run(workbench, 'validate', { quiet: true }); return true; }
      return false;
    }
    ...
    function injectStyles() {
      ...
      style.textContent += `.spain-id-workbench{max-width:100%;overflow-x:hidden}.spain-id-workbench .workbench-form-heading{margin-bottom:14px}.spain-id-workbench .field-grid{grid-template-columns:minmax(260px,.9fr) minmax(240px,.55fr) minmax(240px,.55fr);align-items:start;gap:14px}.spain-id-workbench .field{gap:6px}.spain-id-workbench .field:has(textarea){grid-column:auto}.spain-id-workbench textarea[name="input"]{min-height:82px;max-height:128px;resize:vertical;font-size:1.02rem;line-height:1.35;font-family:ui-monospace,SFMono-Regular,Menlo,monospace}.spain-id-workbench .button-row{gap:10px;margin-top:14px}.spain-id-workbench button,.spain-id-workbench .button,.spain-source-row a,.spain-source-grid a,.spain-mini-copy{transition:border-color .16s ease,background-color .16s ease,color .16s ease,box-shadow .16s ease;transform:none!important}.spain-id-workbench button:hover,.spain-id-workbench .button:hover,.spain-source-row a:hover,.spain-source-grid a:hover,.spain-mini-copy:hover{border-color:rgba(193,18,31,.36);background:rgba(193,18,31,.06);box-shadow:inset 0 0 0 1px rgba(193,18,31,.12),0 8px 22px rgba(15,23,42,.06)}.spain-id-workbench button:focus-visible,.spain-id-workbench .button:focus-visible,.spain-source-row a:focus-visible,.spain-source-grid a:focus-visible,.spain-mini-copy:focus-visible{outline:none;box-shadow:inset 0 0 0 3px rgba(37,99,235,.22)}.spain-sample-deck{grid-column:1/-1;border:1px solid rgba(193,18,31,.16);background:linear-gradient(135deg,rgba(193,18,31,.05),rgba(245,158,11,.04));border-radius:12px;padding:12px 14px;margin-bottom:2px}.spain-sample-row{display:flex;flex-wrap:wrap;gap:8px}.spain-sample-row button{border:1px solid rgba(15,23,42,.12);background:#fff;color:#334155;border-radius:999px;padding:8px 11px;font-weight:800;cursor:pointer}.spain-source-row{display:flex;flex-wrap:wrap;gap:8px;margin-top:10px}.spain-source-row a,.spain-source-grid a{display:inline-flex;align-items:center;min-height:34px;padding:7px 10px;border-radius:999px;border:1px solid #dbeafe;background:#f8fbff;color:#1d4ed8;text-decoration:none;font-weight:800;font-size:.78rem}.spain-panel{margin-top:16px;gap:14px}.spain-timeline{padding:14px}.spain-timeline-track{margin-top:14px}.spain-summary-grid{grid-template-columns:repeat(4,minmax(0,1fr));gap:10px}.spain-summary-card{min-height:78px;background:linear-gradient(180deg,#fff,#fbfdff);box-shadow:0 12px 28px rgba(15,23,42,.04)}.spain-section-card{max-width:100%;overflow:hidden;box-shadow:0 10px 26px rgba(15,23,42,.035)}.spain-section-title{display:flex;align-items:center;justify-content:space-between;gap:12px}.spain-anatomy-table{width:100%;overflow-x:auto}.spain-anatomy-table table{width:100%;min-width:0;border-collapse:collapse;table-layout:fixed;font-size:.86rem}.spain-anatomy-table th{background:#f5f7fb;color:#0f172a;text-align:left;font-weight:900}.spain-anatomy-table th,.spain-anatomy-table td{border-bottom:1px solid #e2e8f0;padding:10px 12px;vertical-align:top;overflow-wrap:anywhere}.spain-anatomy-table code{font-family:ui-monospace,SFMono-Regular,Menlo,monospace;white-space:normal;overflow-wrap:anywhere;color:#0f172a}.spain-source-grid{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:10px}.spain-sources-card p{margin:12px 0 0;color:#64748b;font-size:.92rem;line-height:1.45}.spain-traps-card ul{margin:0;padding-left:18px;color:#64748b;font-size:.8rem;line-height:1.4}.spain-traps-card li+li{margin-top:4px}.spain-mini-copy{border:1px solid #dbeafe;background:#f8fbff;color:#1d4ed8;border-radius:999px;padding:7px 11px;font-weight:900;font-size:.78rem;cursor:pointer}.spain-code{white-space:pre;max-width:100%;max-height:520px}.spain-dev-card{margin-bottom:6px}.spain-compact-section{margin-top:0}@media (max-width:1180px){.spain-id-workbench .field-grid{grid-template-columns:repeat(2,minmax(0,1fr))}.spain-id-workbench .field:has(textarea){grid-column:1/-1}}@media (max-width:980px){.spain-summary-grid,.spain-source-grid{grid-template-columns:repeat(2,minmax(0,1fr))}.spain-id-workbench .field-grid{grid-template-columns:1fr}}@media (max-width:640px){.spain-summary-grid,.spain-source-grid{grid-template-columns:1fr}.spain-section-title{align-items:flex-start;flex-direction:column}.spain-sample-row{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:7px}.spain-sample-row button{width:auto;min-height:36px;padding:7px 6px;border-radius:10px;font-size:.72rem;line-height:1.15}.spain-id-workbench textarea[name="input"]{min-height:78px}}`;
      document.head.appendChild(style);
    }

    function createStandaloneWorkbench(host) {
      const shell = `<div class="workbench-form-heading"><h3>Spain ID Gold Workbench</h3><p>Local DNI, NIE, NIF, legacy CIF, and ES VAT syntax lab.</p></div><div class="field-grid"><label class="field"><span>ID input</span><textarea name="input" rows="3" placeholder="00000000T"></textarea></label></div><div class="button-row"><button type="button" class="button button-primary" data-spain-run="validate">Validate</button><button type="button" class="button button-secondary" data-spain-copy="normalized">Copy normalized</button><button type="button" class="button button-secondary" data-spain-copy="json">Copy developer JSON</button><button type="button" class="button button-secondary" data-spain-run="clear">Clear</button></div><div class="tool-feedback" data-tool-feedback></div><textarea class="output-field" aria-hidden="true"></textarea><div data-advanced-panel></div>`;
      const form = host.matches('form') ? host : null;
      if (form) {
        form.classList.add('browser-workbench', 'spain-standalone-shell');
        form.setAttribute('novalidate', '');
        form.innerHTML = shell;
      } else {
        host.innerHTML = `<form class="browser-workbench spain-standalone-shell" novalidate>${shell}</form>`;
      }
      const mountedForm = form || host.querySelector('form');
      if (!mountedForm) return null;
      const feedback = mountedForm.querySelector('[data-tool-feedback]');
      const output = mountedForm.querySelector('.output-field');
      const workbench = {
        form: mountedForm,
        lastResult: null,
        primaryInput: function () { return mountedForm.querySelector('textarea[name="input"]'); },
      ...
      onMount(workbench);
      mountedForm.addEventListener('click', function (event) {
