...
ValidoHub now has a Node-owned `/guides/` reference layer generated by `scripts/build-reference-guides.mjs` and `npm run build:guides`. This is a support and SEO layer for high-signal live workbenches, not a replacement for tools and not a thin page factory for all generated routes.

The first guide batch covers 45 practical pages: 15 global formats/APIs and 30 local identifier, tax, banking, payment, and fixture workflows. Each guide page is short, tool-first, localized through the production localization pass, exposes browser-checkable evidence, official-boundary notes, integration traps, and links directly to the live workbench.

Guides are generated for configured production locales and appended to `generated/validohub/sitemap.xml` by the scoped guide build. Future guide expansion should remain value-driven: add guides for workflows with real search and integration value, not every route.
